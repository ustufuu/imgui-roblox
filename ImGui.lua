...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:Checkbox(Config)
		...
		if WindowConfig and WindowConfig._Registry and Config.Label then
			WindowConfig._Registry[Config.Label] = {
				Save = function() return Value end,
				Load = function(v) Config:SetTicked(v) end,
			}
		end
	...
	function ContainerClass:InputText(Config)
		...
		if WindowConfig and WindowConfig._Registry and Config.Label then
			WindowConfig._Registry[Config.Label] = {
				Save = function() return TextBox.Text end,
				Load = function(v) Config:SetValue(v) end,
			}
		end
	...
	function ContainerClass:Slider(Config)
		...
		local function ApplyScreenX(screenX)
			if Config.ReadOnly then return end
			if not Dragging then return end
			local LeftPos = Slider.AbsolutePosition.X
			local Percentage = (screenX - LeftPos) / Slider.AbsoluteSize.X
		...
		if WindowConfig and WindowConfig._Registry and Config.Label then
			WindowConfig._Registry[Config.Label] = {
				Save = function() return Config.Value end,
				Load = function(v) Config:SetValue(v) end,
			}
		end
...
function ImGui:Dropdown(Config)
	...
	local StepConnection = RunService.RenderStepped:Connect(function()
		...
		Selection.Position = UDim2.fromOffset(Pos.X + Padding, Pos.Y + Size.Y)
	end)
...
function ImGui:ApplyDraggable(Frame: Frame, Header: Frame)
	...
	local function BeginDrag(screenX, screenY)
		Dragging = true
		DragStartScreen = Vector2.new(screenX, screenY)
		DragStartPos = Frame.Position
	end

	local function UpdateDrag(screenX, screenY)
		if not Dragging then return end
		local delta = Vector2.new(screenX, screenY) - DragStartScreen
	...
	Header.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			BeginDrag(input.Position.X, input.Position.Y)
		elseif input.UserInputType == Enum.UserInputType.Touch then
			if not Dragging then
				ActiveTouch = input
				BeginDrag(input.Position.X, input.Position.Y)
...
function ImGui:ApplyResizable(MinSize, Frame: Frame, Dragger: TextButton, Config)
	...
	local ActiveTouch = nil
	local ResizeStartScreen = nil
	local ResizeStartSize = nil

	local function BeginResize(screenX, screenY)
		Resizing = true
		ResizeStartScreen = Vector2.new(screenX, screenY)
		ResizeStartSize = Frame.AbsoluteSize
	end

	local function UpdateResize(screenX, screenY)
		if not Resizing then return end
		local delta = Vector2.new(screenX, screenY) - ResizeStartScreen
		local newSize = UDim2.fromOffset(
			math.max(MinSize.X, ResizeStartSize.X + delta.X),
			math.max(MinSize.Y, ResizeStartSize.Y + delta.Y)
		)
		Frame.Size = newSize
		if Config then
			Config.Size = newSize
	...
	local function EndResize()
		Resizing = false
		ActiveTouch = nil
		ResizeStartScreen = nil
		ResizeStartSize = nil
	end

	Dragger.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			BeginResize(input.Position.X, input.Position.Y)
		elseif input.UserInputType == Enum.UserInputType.Touch then
	...
	UserInputService.InputChanged:Connect(function(input)
		if not Resizing then return end
		if input.UserInputType == Enum.UserInputType.MouseMovement then
			UpdateResize(input.Position.X, input.Position.Y)
		elseif input.UserInputType == Enum.UserInputType.Touch then
			if input == ActiveTouch then
				UpdateResize(input.Position.X, input.Position.Y)
			end
...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Remove white outline around window
...
local function GetSavedConfigs()
	local ok, Files = pcall(listfiles, "imgui_configs")
	if not ok or not Files then return {} end
	local Names = {}
	for _, Path in next, Files do
		local Name = Path:match("([^/\\]+)$")
		if Name and Name:match("%.json$") then
			local DisplayName = Name:gsub("%.json$", "")
...
local function EnsureConfigDir()
	pcall(function()
		if not isfolder("imgui_configs") then
			makefolder("imgui_configs")
		end
	end)
end

function ImGui:InjectConfigTab(WindowConfig, WindowObj)
	if WindowConfig.TabsBar == false then return end

	local DefaultFile = (WindowConfig.ConfigFile or WindowConfig.Title or "imgui"):gsub("%.json$", "")

	task.defer(function()
		local Registry = WindowConfig._Registry

		local ConfigTab = WindowObj:CreateTab({ Name = "Config" })

		local SaveNameInput = ConfigTab:InputText({
			Label = "Save As",
			Value = DefaultFile,
			PlaceHolder = "profile name",
		})

		local SavedFiles = GetSavedConfigs()
		local SelectedFile = SavedFiles[1] or DefaultFile

		local LoadCombo = ConfigTab:Combo({
			Label = "Load Config",
			Placeholder = #SavedFiles > 0 and "Select..." or "No saves found",
			Items = SavedFiles,
		...
		Row:Button({
			...
			Callback = function()
				...
				EnsureConfigDir()
				local Data = {}
				for Label, Entry in next, Registry do
					Data[Label] = Entry.Save()
				end
				pcall(writefile, "imgui_configs/" .. Name .. ".json", HttpService:JSONEncode(Data))
				local Updated = GetSavedConfigs()
				LoadCombo.Items = Updated
			end,
		})
		...
		Row:Button({
			...
			Callback = function()
				...
				local ok, Raw = pcall(readfile, "imgui_configs/" .. SelectedFile .. ".json")
				if not ok or not Raw or Raw == "" then return end
				local ok2, Data = pcall(function() return HttpService:JSONDecode(Raw) end)
				if not ok2 or typeof(Data) ~= "table" then return end
				for Label, Value in next, Data do
					local Entry = Registry[Label]
