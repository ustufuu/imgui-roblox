...
--// ImGui-like compact font. Roblox's Code font has a sharper, less rounded appearance.
local function ApplyImGuiFont(Root)
...
function ImGui:Tween(Instance: GuiObject, Props: SharedTable, tweenInfo, NoAnimation: false)
	--// Static UI: no TweenService / no interpolation.
	for Property, Value in next, Props do
		pcall(function()
			Instance[Property] = Value
		end)
	end
	local Proxy = {Completed = {}}
	function Proxy.Completed:Connect(Callback)
		Callback()
		return {Disconnect = function() end}
	end
	function Proxy:Play() end
	return Proxy
end
...
function ImGui:HeaderAnimate(Header: Instance, Animation, Open, TitleBar: Instance, Toggle)
	local ToggleButton = Toggle or TitleBar.Toggle.ToggleButton
	pcall(function() ToggleButton.Rotation = Open and 90 or 0 end)
	...
	local ContentSize = UIListLayout and UIListLayout.AbsoluteContentSize or Vector2.zero
	if UIPadding then
		ContentSize = Vector2.new(ContentSize.X, ContentSize.Y + UIPadding.PaddingTop.Offset + UIPadding.PaddingBottom.Offset)
	end
	Container.AutomaticSize = Enum.AutomaticSize.None
	Container.Size = UDim2.new(1, -10, 0, Open and ContentSize.Y or 0)
...
function ImGui:ApplyWindowSelectEffect(Window: GuiObject, TitleBar)
	--// Static UI: no hover animation and no window border.
...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Remove the outer white window outline.
	for _, Obj in next, Window:GetDescendants() do
	...
	function WindowConfig:Close()
		...
		--// Полный destroy всех гуишек библиотеки (не просто скрытие)
		ImGui.Windows[Window] = nil

		if ImGui.ScreenGui then
			ImGui.ScreenGui:Destroy()
			ImGui.ScreenGui = nil
			_G["DepsoImGuiScreenGui"] = nil
		end
		if ImGui.FullScreenGui then
			ImGui.FullScreenGui:Destroy()
			ImGui.FullScreenGui = nil
			_G["DepsoImGuiFullScreenGui"] = nil
		end
	...
	function WindowConfig:ShowTab(TabClass: SharedTable)
		...
		--// Static tab switching: no fade/slide.
...
--// Уничтожаем гуишки, оставшиеся от предыдущего выполнения скрипта
--// (иначе при повторном loadstring старое окно остаётся висеть поверх нового
--// и создаётся иллюзия, что текст/лейблы не обновляются)
local function DestroyLeftoverGui(CacheKey)
	local Old = _G[CacheKey]
	if Old then
		pcall(function() Old:Destroy() end)
	end
end
DestroyLeftoverGui("DepsoImGuiScreenGui")
DestroyLeftoverGui("DepsoImGuiFullScreenGui")
...
--// Сохраняем ссылки в _G, чтобы следующий loadstring мог их найти и убрать
_G["DepsoImGuiScreenGui"] = ImGui.ScreenGui
_G["DepsoImGuiFullScreenGui"] = ImGui.FullScreenGui
