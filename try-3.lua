...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:Checkbox(Config)
		...
		-- Override blue tint to authentic ImGui dark gray tint
		Tick.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
		Tickbox.BackgroundColor3 = Color3.fromRGB(37, 37, 38)
	...
	function ContainerClass:InputText(Config)
		...
		-- Override background color to authentic ImGui Input Dark style
		TextBox.BackgroundColor3 = Color3.fromRGB(29, 29, 31)
	...
	function ContainerClass:Slider(Config)
		...
		-- Override Slider Grab color from blue to Dear ImGui classic Dark Gray Accent
		Grab.BackgroundColor3 = Color3.fromRGB(66, 150, 250)
		Slider.BackgroundColor3 = Color3.fromRGB(37, 37, 38)
...
function ImGui:Unload()
	for Window, WindowConfig in next, ImGui.Windows do
		pcall(function() WindowConfig:Unload() end)
	end
	if ImGui.ScreenGui then ImGui.ScreenGui:Destroy() end
	if ImGui.FullScreenGui then ImGui.FullScreenGui:Destroy() end
	_G["DepsoImGui"] = nil
end
...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Стилизация крестика закрытия под оригинальный Dear ImGui
	CloseButton.Text = "x"
	...
	function WindowConfig:Unload()
		local Callback = WindowConfig.CloseCallback
		if Callback then
			pcall(Callback, WindowConfig)
		end
		
		ImGui.Windows[Window] = nil
		Window:Destroy()
		
		-- Если это было последнее окно, чистим ScreenGui интерфейсы полностью
		local remainingCount = 0
		for _ in next, ImGui.Windows do
			remainingCount += 1
		end
